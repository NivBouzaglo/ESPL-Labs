...
char* getOldest(circularQueue* history){
    return history->arr[history->oldest];
}

char* getIndex(circularQueue* history, int n) {
    if (n < HISTLEN) {
        int index = my_history->newest - n;
        if (index < 0)
            index = HISTLEN + index - 1;
...
void addHistory(circularQueue* history, char* new){
    ...
    if(index == history->oldest)
        history->oldest = (index + 1) % HISTLEN;
...
void printHistory(circularQueue* history,int n){
    int curr = history->newest;
    while(n > 0){
        printf("%s", history->arr[curr]);
        curr = (curr - 1) ;
        if(curr < 0){
            curr = HISTLEN - 1;
        }
...
int specialCommand(cmdLine* command){
    ...
    if(strcmp(command->arguments[0], "history") == 0){
        isSpecial = 1;
        int n = my_history->newest - my_history->oldest;
    ...
    if(strcmp(command->arguments[0], "!!") == 0){
        ...
        char* cmdStr = getIndex(my_history, n);
        printf("%s\n", cmdStr);
        struct cmdLine* cmd = parseCmdLines(cmdStr);
        execute(cmd, debug);
    }
    ...
    if(!specialCommand(pCmdLine)){
        ...
        if(!(child=fork())){
            if(pCmdLine->next != NULL && pCmdLine->next->outputRedirect != NULL){
                close(STDOUT);
                outputFile = open(pCmdLine->next->outputRedirect, O_CREAT | O_WRONLY);
                if(outputFile < 0)
                    Error("");
            }
            if(pCmdLine->inputRedirect != NULL) {
                close(STDIN);
                inputFile = open(pCmdLine->inputRedirect, O_RDONLY);
                if(inputFile < 0)
                    Error("");
            }
