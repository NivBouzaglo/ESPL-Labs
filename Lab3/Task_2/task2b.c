...
void kill_virus(char* fileName, int signatureOffset ,int signatureSize)
{
    ...
    char * change = (char*)calloc(signatureSize,sizeof(char));
    for(int i=0;i<signatureSize;i++)
        change[i]= 0x90;
    fwrite(change,1,signatureSize,fp);
    free(change);
    fclose(fp);
}
    ...
    while (1)
    {
        ...
        if(option == 1){
           ...
           fclose(input);
            ...
            FILE* fp = fopen("infected" ,"r+");
            char buff[100000];
            fread(buff ,10000 , sizeof(char) ,fp);
            fseek(fp ,0,SEEK_END);
            int size = ftell(fp);
            fseek(fp,0,SEEK_SET);
        ...
        {
            puts("Enter the file you need to fix: ");
            char name[200];
            char idxOfVirus[200];
            char size[200];
            fgets(name ,200,stdin);
            name[strcspn(name,"\n")]=0;
            puts("please enter the index of the virus: ");
            fgets(idxOfVirus,199,stdin);
            name[strcspn(idxOfVirus,"\n")]=0;
             int index = atoi(idxOfVirus);
            puts("please enter the size of the virus: ");
            fgets(size,199,stdin);
            name[strcspn(size,"\n")]=0;
            int s = atoi(size);
            kill_virus(name,index,s);
        }    
        else if(option == 5){
